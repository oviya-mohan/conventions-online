...
class Make_Choice(Page):
    ...
    #position randomization is taken care of in creating_session
    @staticmethod
...
page_sequence = [WaitForPlayer, Make_Choice, Show_Choice,
...
def custom_export(players):
    #header row #1
    yield['participant_number', 'participant_code', 'answer']
    for p in players:
        answer = p.Q
        #write only for last (answer) round
        if p.round_number == Constants.num_rounds:
            yield[p.id_in_group, p.participant.code, answer]
    ...
    for p in players:
        ...
        left_stimuli, right_stimuli, response, loser, p.answered_time,
